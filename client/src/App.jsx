...
const NewspaperIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none"
    ...
    <path d="M4 22a2 2 0 0 0-2-2V4a2 2 0 0 0 2-2h4" />
    <path d="M18 22V4" /><path d="M12 22V4" />
    <path d="M8 12h4" /><path d="M8 17h4" /><path d="M8 7h4" />
  </svg>
);
  ...
  <header className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg shadow-sm p-4 sticky top-0 z-30">
    <div className="container mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">News Hub</h1>
      </div>
...
const ArticleCard = ({ article, onArticleClick }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 cursor-pointer flex flex-col group"
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto relative" onClick={(e) => e.stopPropagation()}>
        ...
        <img src={article.image_url || placeholderImage} alt={article.title} className="w-full h-72 object-cover"
          onError={(e) => { e.target.onerror = null; e.target.src = placeholderImage; }} />
        <div className="p-8">
          <p className="text-sm text-blue-500 dark:text-blue-400 mb-2 font-semibold">{article.source}</p>
          ...
          <a href={article.url} target="_blank" rel="noopener noreferrer"
            className="inline-block bg-blue-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700 transition-all duration-300 transform hover:scale-105">
            Read Full Story
          </a>
        </div>
      </div>
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        ...
        const normalized = (response.data.articles || response.data || []).map((a, idx) => ({
          id: a.id || idx,
          ...
          articlesToDisplay.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {articlesToDisplay.map((article) => (
                <ArticleCard key={article.id} article={article} onArticleClick={setSelectedArticle} />
              ))}
            </div>
          ) : <EmptyState />
        )}
